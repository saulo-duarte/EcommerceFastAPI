...
def test_user_create_valid():
    ...
    user = UserCreate(
        email="user@example.com", full_name=" John Doe ", password=password
    )
...
def test_user_create_invalid_full_name():
    password = Password(raw="Valid1@Password")
    with pytest.raises(
        ValueError, match="Full name must contain only letters and spaces"
    ):
        UserCreate(email="user@example.com", full_name="John123", password=password)


def test_user_update_partial():
    update = UserUpdate(full_name="  Jane Smith  ", is_active=False)
